...
class OTWBMLVCExtendAddonClass{
	...
	public function renderShortcode( $params ){
		...
		if( isset( $params['otw_blog_list'] ) && intval( $params['otw_blog_list'] ) ){
			echo do_shortcode( '[otw-bm-list id="'.esc_attr( $params['otw_blog_list'] ).'"]' );
		}
	}
	...
	public function integrateWithVC(){
		...
		if( $this->has_vc ){
			...
			vc_map( array(
				"name" => esc_html__("Blog Manager", 'otw_bml'),
				"description" => esc_html__("Select a post list created with Blog Manager plugin", 'otw_bml'),
				"base" => "otw_bm_vc",
				"class" => "",
				"controls" => "full",
				...
				"category" => esc_html__('Blog Manager', 'otw_bml'),
				"params" => array(
					array(
						'type' => 'dropdown',
						'holder' => 'div',
						'class' => '',
						'heading' => esc_html__( 'Blog list', 'otw_bml'),
						'param_name' => 'otw_blog_list',
						'value' => $options,
						'description' => esc_html__( 'Description for blog list.', 'otw_bml')
					)
				)
			) );
		}
