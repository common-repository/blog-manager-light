...
class OTWBML_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array(
				'description' => esc_html__('With the help of this widget you can add your created Widget Lists to the sidebars', 'otw_bml')
			)
		);
	...
	function form ( $instance ) {
		...
		$htmlForm  = '<p>';
		$htmlForm .= '<label for="'.esc_attr( $field_title ).'">'. esc_html__('Title:', 'otw_bml') .'</label><br>';
		...
		$htmlForm .= '<label for="'.esc_attr( $field_id ).'">'. esc_html__('OTW Blog List Widget:', 'otw_bml') .'</label><br>';
		...
		if( !empty( $widgetID ) ) {
			...
			if ( !empty( $otw_bm_options ) ) {
				...
				global $wp_filesystem;
					...
					if( $wp_filesystem->exists(SKIN_BML_PATH . 'otw-bm-list-'.$widgetID.'-custom.css') ) {
