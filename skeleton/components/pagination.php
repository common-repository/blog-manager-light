...
if( $this->listOptions['show-pagination'] == 'pagination' && !empty( $this->listOptions['posts_limit_page'] ) ) :
  ...
  if( $otw_bm_posts->max_num_pages > 1 ) :
    ...
    if( otw_get( 'post_id', false ) && preg_match( "/^\d+$/", otw_get( 'post_id', '' ) ) ){
    		...
    		if( otw_get( 'post_id', '' ) != $this->listOptions['id'] ){
    			$currentPage = 1;
    		}
    }
    ...
    $pagedArgs = array(
      ...
      'mid_size'      => 3,
      'prev_next'     => true,
      'prev_text'     => esc_html__('« Previous', 'otw_bml'),
      'next_text'     => esc_html__('Next »', 'otw_bml'),
      'type'          => 'array',
 ...
 <div class="otw-twentyfour otw-columns">
    ...
    <div class="js-pagination_container">
      ...
      <div class="<?php echo $paginationLoadMore;?> js-otw_blog_manager-load-more">
        <a href="<?php echo esc_attr( $ajaxURL );?>" data-empty="No more posts to load." data-isotope="true"><?php esc_html_e('Load More...', 'otw_bml');?></a>
      </div>
    </div> <!-- End Pagination -->
...
<div class="otw_blog_manager-pagination hide">
  <a href="<?php echo esc_attr( $ajaxURL );?>" data-empty="No more posts to load." data-isotope="true">2</a>
</div>
<!-- End Infinite Scroll -->
<?php endif; ?>